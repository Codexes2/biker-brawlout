...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('friday_ride_loop'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('friday_ride'), 0.7);
			}
		}
		...
		#end
			gfDance.animation.addByPrefix('dance', 'gfDance', 24, false);
	
		gfDance.antialiasing = ClientPrefs.globalAntialiasing;
		add(gfDance);

		demoSpr = new FlxSprite(920, 0);
		demoSpr.frames = Paths.getSparrowAtlas('demoBumpin');
		demoSpr.antialiasing = FlxG.save.data.antialiasing;
		demoSpr.animation.addByPrefix('bump', 'demo bumpin', 24, false);
		demoSpr.updateHitbox();
		add(demoSpr);
		...
		if(!closedState) {
			switch (curBeat)
			{
				case 0:
					deleteCoolText();
				case 8:
					createCoolText(['Xiang', 'CamoREX', 'Codexes']);
				case 10:
					addMoreText('present');
				case 12:
					deleteCoolText();
				...
				case 32:
					deleteCoolText();
					createCoolText(['Friday Night Funkin', 'in']);
				case 34:
					addMoreText('Biker Brawlout');
				case 36:
					deleteCoolText();
					addMoreText('Friday Ride');
				case 38:
					addMoreText('by CamoREX');
				case 40:
					skipIntro();
			}
		}
